        ...
        init: function() {
            ...
            ig.io.sockets.on('connection', function(socket) {
                ...
                socket.on('init', function(x, y) {
                    ...
                    // Prevent client from initializing multiple times.
                    if(typeof game.lights[socket.id] !== 'undefined') {
                        console.log("Received init call from already-initialized client.");
                        return;
                    }
                ...
                socket.on('update-pos', function(x, y) {
                    ...
                    // Prevent client from sending unauthorized updates.
                    if(typeof game.lights[socket.id] === 'undefined') {
                        console.log("Received update from non-initialized client.");
                        return;
                    }
